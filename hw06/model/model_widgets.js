...
exports.modify = function(data, callback){
    if(!(data && data.data.name && data.data.price && data.id)) return callback(new Error('Error data'));
    console.log(data.id, data.data)
    exports.find(data.id, (err, dataFound) => {
        console.log(111);
        if (err) return res.send('Error modify widget!');
        dataFound.name = data.data.name;
        dataFound.price = data.data.price;
        dataFound.desc = data.data.desc;        
    })
    callback(null, null);
}; 
